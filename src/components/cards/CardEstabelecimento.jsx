...
const CardEstab = ({ estabelecimento }) => {
  const { id, nome, categoria, notaGeral, qtdAvaliacoes, rua, numero, bairro, cidade, estado, cep } =
    estabelecimento || {};

  const { categoriaMapeada, notaFormatada, localizacao } = formataInfosEstabelecimento(
    categoria,
    notaGeral,
    rua,
    numero,
  ...
  return (
    <Link to={`/estabelecimento/${id}`} className="block w-full text-current no-underline">
